...
- `minNoiseDelta`: If the spectral power varies very little across frequencies around the target frequency, it may be desirable to introduce a minimal deviation for computing the "below" and "above" thresholds. This can be done by setting this parameter > 0 (e.g., a value of 0.1 dB corresponds to a minimal power ratio for thresholds of about $10 ^(1/20) \approx 1.12$, i.e., around +/- 10%).
